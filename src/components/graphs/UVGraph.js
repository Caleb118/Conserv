...
class UVGraph extends React.Component {
        ...
        axios.get(`https://app.conserv.io/data/api/health/db`)
          ...
          .then(() => {
            ...
            dataArr.map((currentValue) => {
                currentValue.range_uv = [currentValue.min_uv1, currentValue.max_uv1];
                return null;
            });
        ...
        if (active) {
            ...
            <div className="card border-primary mb-3">
                ...
                <div className="card-body">
                    <p className="card-text"><b>Average Lux:</b> { !payload[0].value ? 0 : payload[0].value }</p>
                    <p className="card-text"><b>Minimum Lux:</b> { !payload[1].value ? 0 : payload[1].value }</p>
                    <p className="card-text"><b>Maximum Lux:</b> { !payload[2].value ? 0 : payload[2].value }</p>
                </div>
            </div>
            );
        }
        ...
        renderLineChart = (
            <ResponsiveContainer width={'90%'} aspect={2}>
                <ComposedChart data={smalldata}>
                    ...
                    <Line type="monotone" connectNulls dataKey="avg_uv1" stroke="#ff0000" name="Average Lux" />
                    <Line type="monotone" connectNulls dataKey="min_uv1" stroke="#ff4a4a" name="Minimum Lux"/>
                    <Line type="monotone" connectNulls dataKey="max_uv1" stroke="#ff4a4a" name="Maximum Lux"/>
                    <Area type="monotone" connectNulls dataKey="range_uv"  fill="#ffb0b0" stroke="#ffb0b0" name="UltraVoilet Range" />
