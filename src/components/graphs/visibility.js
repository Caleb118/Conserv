        ...
        axios.get(`https://app.conserv.io/data/api/health/db`)
          ...
          .then(() => {
            ...
            dataArr.map((currentValue) => {
                currentValue.range_vis = [currentValue.min_vis, currentValue.max_vis];
                return null;
            });
        ...
        if (active) {
            ...
            <div className="card border-primary mb-3">
                ...
                <div className="card-body">
                    <p className="card-text"><b>Average Visibility:</b> { !payload[0].value ? 0 : payload[0].value }</p>
                    <p className="card-text"><b>Minimum Visibility:</b> { !payload[1].value ? 0 : payload[1].value }</p>
                    <p className="card-text"><b>Maximum Visibility:</b> { !payload[2].value ? 0 : payload[2].value }</p>
                </div>
            </div>
            );
        }
        ...
        renderLineChart = (
            <ResponsiveContainer width={'90%'} aspect={2}>
                <ComposedChart data={smalldata}>
                    ...
                    <Line type="monotone" connectNulls dataKey="avg_vis" stroke="#ff0000" name="Average Visibility" />
                    <Line type="monotone" connectNulls dataKey="min_vis" stroke="#ff4a4a" name="Minimum Visibility"/>
                    <Line type="monotone" connectNulls dataKey="max_vis" stroke="#ff4a4a" name="Maximum Visibility"/>
                    <Area type="monotone" connectNulls dataKey="range_vis"  fill="#ffb0b0" stroke="#ffb0b0" name="Visibility Range" />
